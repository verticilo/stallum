...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.FUNCOES" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/funcoes/nova"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="nova" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/funcoes/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoFuncao.${ordenacao}" /></option>
						</c:forEach>
					</select>
		...
		<div class="cabecalho">
			<div style="width: 200px;"><fmt:message key="funcao.nome" /></div>
			<div style="width: 150px;"><fmt:message key="funcao.sindicato" /></div>
			<div style="width: 100px;"><fmt:message key="funcao.salario" /></div>
			<div style="width: 110px;"><fmt:message key="dissidio.data" /></div>
		</div>
		
		<c:forEach items="${listaFuncoes}" var="funcao">
			<div class="linha">
				<div style="width: 200px;"><a href="<c:url value="/funcoes/${funcao.id}/editar"/>">${funcao.nome}</a></div>
				<div style="width: 150px;">${funcao.sindicato.nome}</div>
				<div style="width: 100px;"><fmt:formatNumber type="NUMBER" value="${funcao.salario}" minFractionDigits="2" /></div>
				<div style="width: 110px;"><fmt:formatDate type="date" dateStyle="MEDIUM" value="${funcao.dataUltimoDissidio}" /></div>
				<div style="width: auto;">
					<a href="<c:url value="/funcoes/${funcao.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
				</div>
			</div>		
		</c:forEach>
