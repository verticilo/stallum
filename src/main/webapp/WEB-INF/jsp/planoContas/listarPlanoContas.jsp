...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.PLANO_CONTAS" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/planoContas/novo"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/planoContas/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoContaGerencial.${ordenacao}" /></option>
						</c:forEach>
					</select>
		...
		<div class="cabecalho">
			<div style="width: 100px;"><fmt:message key="contaGerencial.numero" /></div>
			<div style="width: 400px;"><fmt:message key="contaGerencial.nome" /></div>
		</div>
		
		<c:forEach items="${listaPlanoContas}" var="contaGerencial">
			<div class="linha">
				<div style="width: 100px;">${contaGerencial.numero}</div>
				<c:if test="${contaGerencial.totalizadora}">
					<div style="width: 400px; font-weight: bold;">${contaGerencial.nome}</div>
				</c:if>
				<c:if test="${empty contaGerencial.totalizadora or not contaGerencial.totalizadora}">
					<div style="width: 400px;">&nbsp;&nbsp;&nbsp;${contaGerencial.nome}</div>
				</c:if>
				<div style="width: auto;">
					<a href="<c:url value="/planoContas/${contaGerencial.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
				</div>
			</div>		
		</c:forEach>
