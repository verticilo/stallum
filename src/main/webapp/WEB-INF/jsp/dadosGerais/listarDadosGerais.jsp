...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.DADOS_GERAIS" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/dadosGerais/novo"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/dadosGerais/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoDadosGerais.${ordenacao}" /></option>
						</c:forEach>
					</select>
		...
		<div class="cabecalho">
			<div style="width: 350px;"><fmt:message key="dadosGerais.descricao" /></div>
			<div style="width: 100px;"><fmt:message key="dadosGerais.data" /></div>
		</div>
		
		<c:forEach items="${listaDadosGerais}" var="dadosGerais">
			<div class="linha">
				<div style="width: 350px;"><a href="<c:url value="/dadosGerais/${dadosGerais.id}/editar"/>">${dadosGerais.descricao}</a></div>
				<div style="width: 100px;"><fmt:formatDate type="date" dateStyle="MEDIUM" value="${dadosGerais.data}" /></div>
				<div style="width: auto;">
					<a href="<c:url value="/dadosGerais/${dadosGerais.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
				</div>
			</div>		
		</c:forEach>
