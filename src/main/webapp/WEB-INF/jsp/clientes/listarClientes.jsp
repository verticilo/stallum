...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.CLIENTES" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/clientes/novo"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/clientes/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoCliente.${ordenacao}" /></option>
						</c:forEach>
					</select>
					
					<c:if test="${sessaoUsuario.administrador}">
						<input type="checkbox" name="filtro.removidos" <c:if test="${filtro.removidos}">checked="checked"</c:if> /> <fmt:message key="filtroCliente.removidos" />
					</c:if>
		...
		<div class="cabecalho">
			<div style="width: 220px;"><fmt:message key="cliente.razaoSocial" /></div>
			<div style="width: 150px;"><fmt:message key="cliente.nomeCurto" /></div>
			<div style="width: 90px;"><fmt:message key="listarClientes.telefone" /></div>
			<div style="width: 180px;"><fmt:message key="cliente.email" /></div>
		</div>
		
		<c:forEach items="${listaClientes}" var="cliente">
			<div class="linha">
				<div style="width: 220px;"><a href="<c:url value="/clientes/${cliente.id}/editar"/>">${cliente.razaoSocial}</a></div>
				<div style="width: 150px;">${cliente.nomeCurto}</div>
				<div style="width: 90px;">${cliente.ddd1} ${cliente.telefone1}</div>
				<div style="width: 180px; text-transform: lowercase;">${cliente.email}</div>
				<div style="width: auto;">
					<a href="<c:url value="/clientes/${cliente.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
					<c:if test="${sessaoUsuario.administrador and not cliente.ativo}">
						<a href="<c:url value="/clientes/${cliente.id}/editar"/>"><img src="<c:url value="/images/remover.png"/>" width="16" height="16" title="<fmt:message key="recuperar"/>" /></a>
					</c:if>
				</div>
			</div>		
		</c:forEach>
