...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.OBRAS" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/obras/nova"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/obras/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoObra.${ordenacao}" /></option>
						</c:forEach>
					</select>
					
					<c:if test="${sessaoUsuario.administrador}">
						<input type="checkbox" name="filtro.removidos" <c:if test="${filtro.removidos}">checked="checked"</c:if> /> <fmt:message key="filtroObra.removidos" />
					</c:if>
		...
		<div class="cabecalho">
			<div style="width: 250px;"><fmt:message key="obra.nome" /></div>
			<div style="width: 250px;"><fmt:message key="obra.cliente" /></div>
			<div style="width: 100px;"><fmt:message key="obra.status" /></div>
		</div>
		
		<c:forEach items="${listaObras}" var="obra">
			<div class="linha">
				<div style="width: 250px;"><a href="<c:url value="/obras/${obra.id}/editar"/>">${obra.nome}</a></div>
				<div style="width: 250px;">${obra.cliente.nomeCurto}</div>
				<div style="width: 100px;"><fmt:message key="statusObra.${obra.status}" /></div>
				<div style="width: auto;">
					<a href="<c:url value="/obras/${obra.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
					<c:if test="${obra.status == 'REMOVIDA'}">
						<a href="<c:url value="/obras/${obra.id}/editar"/>"><img src="<c:url value="/images/remover.png"/>" width="16" height="16" title="<fmt:message key="recuperar"/>" /></a>
					</c:if>					
					<a href="<c:url value="/obras/${obra.id}/movimentosObra/listar"/>"><img src="<c:url value="/images/medicoes.png"/>" width="16" height="16" title="<fmt:message key="obra.movimentos"/>" /></a>
				</div>
			</div>		
		</c:forEach>
