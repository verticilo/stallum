...
<div class="content">
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/sindicatos/novo"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="nova" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/sindicatos/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoSindicato.${ordenacao}" /></option>
						</c:forEach>
					</select>
		...
		<div class="cabecalho">
			<div style="width: 200px;"><fmt:message key="sindicato.nome" /></div>
			<div style="width: 110px;"><fmt:message key="sindicato.dataDissidio" /></div>
			<div style="width: 100px;"><fmt:message key="sindicato.salarioMinimo" /></div>
		</div>
		
		<c:forEach items="${listaSindicatos}" var="sindicato">
			<div class="linha">
				<div style="width: 200px;"><a href="<c:url value="/sindicatos/${sindicato.id}/editar"/>">${sindicato.nome}</a></div>
				<div style="width: 110px;">${sindicato.dataDissidio}</div>
				<div style="width: 100px;"><c:if test="${sindicato.salarioMinimo}">X</c:if></div>
				<div style="width: auto;">
					<a href="<c:url value="/sindicatos/${sindicato.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
				</div>
			</div>		
		</c:forEach>
