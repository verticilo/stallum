...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.DISSIDIOS" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/dissidios/novo"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/dissidios/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoDissidio.${ordenacao}" /></option>
						</c:forEach>
					</select>
		...
		<div class="cabecalho">
			<div style="width: 220px;"><fmt:message key="funcao.sindicato" /></div>
			<div style="width: 150px;"><fmt:message key="dissidio.data" /></div>
		</div>
		
		<c:forEach items="${listaDissidios}" var="dissidio">
			<div class="linha">
				<div style="width: 220px;"><a href="<c:url value="/dissidios/${dissidio.id}/editar"/>">${dissidio.funcao.sindicato}</a></div>
				<div style="width: 150px;"><fmt:formatDate type="date" dateStyle="MEDIUM" value="${dissidio.data}" /></div>
				<div style="width: auto;">
					<a href="<c:url value="/dissidios/${dissidio.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
				</div>
			</div>		
		</c:forEach>
