...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.CONTAS_CORRENTES" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/contasCorrentes/novo"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/contasCorrentes/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoContaCorrente.${ordenacao}" /></option>
						</c:forEach>
					</select>
		...
		<div class="cabecalho">
			<div style="width: 150px;"><fmt:message key="contaCorrente.nomeBanco" /></div>
			<div style="width: 150px;"><fmt:message key="contaCorrente.numAgencia" /></div>
			<div style="width: 150px;"><fmt:message key="contaCorrente.numConta" /></div>
		</div>
		
		<c:forEach items="${listaContasCorrentes}" var="contaCorrente">
			<div class="linha">
				<div style="width: 150px;">${contaCorrente.nomeBanco}</div>
				<div style="width: 150px;">${contaCorrente.numAgencia}</div>
				<div style="width: 150px;">${contaCorrente.numConta}</div>
				<div style="width: auto;">
					<a href="<c:url value="/contasCorrentes/${contaCorrente.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
				</div>
			</div>		
		</c:forEach>
