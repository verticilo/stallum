...
<div class="content">
	...
	<h3><fmt:message key="listarMedicoes.titulo" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/medicoes/nova"/>"><img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" /></a>
			</div>
			
			<form class="filtro" action="<c:url value="/medicoes/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoMedicao.${ordenacao}" /></option>
						</c:forEach>
					</select>
					
					<input type="checkbox" name="filtro.canceladas" <c:if test="${filtro.canceladas}">checked="checked"</c:if> /> <fmt:message key="filtroMedicao.canceladas" />
		...
		<div class="cabecalho">
			<div style="width: 200px;"><fmt:message key="medicao.obra" /></div>
			<div style="width: 120px;"><fmt:message key="medicao.empresa" /></div>
			<div style="width: 70px;"><fmt:message key="medicao.notaFiscal" /></div>
			<div style="width: 85px;"><fmt:message key="medicao.data" /></div>
			<div style="width: 85px; text-align: right;"><fmt:message key="medicao.valor" /></div>
			<div style="width: 60px; text-align: center;"><fmt:message key="medicao.cancelada" /></div>
		</div>
		
		<c:forEach items="${listaMedicoes}" var="medicao">
			<div class="linha">
				<div style="width: 200px;"><a href="<c:url value="/medicoes/${medicao.id}/editar"/>">${medicao.obra.nomeCurto}</a></div>
				<div style="width: 120px;">${medicao.nomeEmpresa}</div>
				<div style="width: 70px;">${medicao.notaFiscal}</div>
				<div style="width: 85px;"><fmt:formatDate type="date" dateStyle="MEDIUM" value="${medicao.data}" /></div>
				<div style="width: 85px; text-align: right;"><fmt:formatNumber type="NUMBER" value="${medicao.valor}" minFractionDigits="2" /></div>
				<div style="width: 60px; text-align: center;"><c:if test="${medicao.cancelada}">X</c:if></div>
				<div style="width: auto;">
					<a href="<c:url value="/medicoes/${medicao.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>					
				</div>
			</div>		
		</c:forEach>
