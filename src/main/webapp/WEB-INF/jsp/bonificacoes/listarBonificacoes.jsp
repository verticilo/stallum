...
<div class="content">
	...
	<h3><fmt:message key="itensMenu.BONIFICACOES" /></h3>
	...
	<div class="lista">
		...
		<div class="btnFiltro">
			...
			<div>
				<a href="<c:url value="/bonificacoes/nova"/>">
					<img src="<c:url value="/images/novo.png"/>" title="<fmt:message key="novo" />" />
				</a>
			</div>
			
			<form class="filtro" action="<c:url value="/bonificacoes/listar" />" method="post">
				...
				<div>
					...
					<select name="filtro.ordenacao">
						<c:forEach items="${filtro.listaOrdenacao}" var="ordenacao">
							<option value="${ordenacao}" <c:if test="${ordenacao == filtro.ordenacao}">selected="selected"</c:if> ><fmt:message key="ordenacaoBonificacao.${ordenacao}" /></option>
						</c:forEach>
					</select>
		...
		<div class="cabecalho">
			<div style="width: 150px;"><fmt:message key="bonificacao.data" /></div>
		</div>
		
		<c:forEach items="${listaBonificacoes}" var="bonificacao">
			<div class="linha">
				<div style="width: 150px;"><fmt:formatDate type="date" dateStyle="MEDIUM" value="${bonificacao.data}" /></div>
				<div style="width: auto;">
					<a href="<c:url value="/bonificacoes/${bonificacao.id}/editar"/>"><img src="<c:url value="/images/editar.png"/>" width="16" height="16" title="<fmt:message key="editar"/>" /></a>
				</div>
			</div>		
		</c:forEach>
